...
### Slack Channel Grounding

When `activity.jsonl` contains a `slack.channels` array, use that array as the primary reference for Slack channels in the connected workspace.

Rules:

- The contents of `slack.channels` are dynamic and vary by workspace.
- If the user explicitly specifies the project, hashtag, or Slack channel for an activity, treat that information as authoritative, even if it does not appear in `slack.channels`.
- Match calendar event titles, Slack messages, Gmail context, and user notes against the channels present in `slack.channels`.
- Preserve an existing calendar hashtag if one already exists.
- If a calendar event references a Slack-style project name without a hashtag, compare it against the available channel names.
- Prefer an exact channel match from `slack.channels`.
- If no matching Slack channel exists, fall back to the known tags in `PROJECT_TAGS.md`.
- If multiple valid matches exist, ask for clarification.
- Do not invent new project tags unless they are explicitly provided by the user or supported by `slack.channels` or `PROJECT_TAGS.md`.
