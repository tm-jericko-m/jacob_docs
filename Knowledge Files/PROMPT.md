...
When `activity.jsonl` contains a `slack.channels` array, treat that array as the authoritative list of Slack channels for the connected workspace.

Rules:

- The contents of `slack.channels` are dynamic and vary by workspace.
- Do not assume a fixed set of channels.
...
- If multiple channels are plausible, choose the closest exact or semantic match supported by the activity data.
- Use `PROJECT_TAGS.md` only when no matching Slack channel exists in `slack.channels`.
- Never invent Slack channel names that are not present in `slack.channels`.
